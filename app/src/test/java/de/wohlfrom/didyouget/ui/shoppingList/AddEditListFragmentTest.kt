...
class AddEditListFragmentTest {
    ...
    class AddListTest(private val listName: String?) {
        companion object {
            @JvmStatic
            @Parameters(name = "listName={0}")
            fun listNames() = listOf(null, "")
        }
        ...
        fun testAddListSuccess() {
            ...
            coEvery { anyConstructed<ShoppingListRepository>().addShoppingList(any()) } returns Result.Success(
                true
            )
    ...
    class EditListTest {
        ...
        fun testEditListSuccess() {
            mockkConstructor(ShoppingListRepository::class)
            coEvery {
                anyConstructed<ShoppingListRepository>().renameShoppingList(
                    any(),
                    any()
                )
